#This R script calculates enrichment statistics, evaluates predictability, and models co-occurrence networks for In-silico Predicted Motifs (IPMs) against annotated true Transcription Factor Binding Sites (TFBS). 
#The outputs generated by this pipeline correspond to the data and visualizations used for Main Figure 4 and Supplementary Tables 4–5 in your manuscript.
#SMZ 2026-03-01
...
file_path_predictions <- "./studies/deepCIS/ipmArthDAPmotifs/extracted_sites_predictions_chrom_1.csv"
file_path_common_ids  <- "./studies/deepCIS/ipmArthDAPmotifs/extracted_ranges_1.txt"
file_path_true_sites  <- "./studies/deepCIS/ipmArthDAPmotifs/all_binding_sites.csv"
output_file_path      <- "IPM_Enrichment_Strict_Overlap.csv"
...
common_ids <- read.table(file_path_common_ids, sep = " ", header = FALSE, stringsAsFactors = FALSE)
colnames(common_ids) <- c("seq_id", "label")
common_ids <- common_ids %>% distinct(seq_id, label, .keep_all = TRUE)

# 2. Load True Binding Sites
print("Loading True Binding Sites...")
true_binding_sites <- read.csv(file_path_true_sites)

# --- DIAGNOSTIC: CHECK COLUMNS ---
print("Columns in True Binding Sites:")
print(colnames(true_binding_sites))

# !!! CRITICAL FIX: Ensure we have a 'label' column !!!
# If there is no 'label' column, we try to guess it or rename the likely candidate.
# CHANGE "target_label" below to the actual column name if this guess is wrong.
possible_names <- c("label", "Label", "TF", "tf", "name", "Name", "Target", "target", "TF_Name")
matched_col <- intersect(colnames(true_binding_sites), possible_names)

if (length(matched_col) > 0) {
  print(paste("Renaming column", matched_col[1], "to 'label' for consistency."))
  colnames(true_binding_sites)[colnames(true_binding_sites) == matched_col[1]] <- "label"
} else {
  # Fallback: Assume the last column is the label if not found (Risky, but often true for BED-like CSVs)
  print("WARNING: Could not identify TF Name column. Assuming the last column is the label.")
  colnames(true_binding_sites)[ncol(true_binding_sites)] <- "label"
}

# Filter for Chromosome 1
true_binding_sites <- true_binding_sites %>% 
  filter(chr %in% c("1", "Chr1", "chr1"))

gr_true_sites <- makeGRangesFromDataFrame(true_binding_sites, 
...
# 3. Load Predictions
print("Loading Predictions (Chunks)...")
chunk_size <- 100000 
total_rows <- fread(file_path_predictions, select = 1)[, .N]
chunks <- list()
chunk_number <- 1
skip <- 0
...
while(skip < total_rows) {
  rows_left <- total_rows - skip
  nrows_to_read <- min(chunk_size, rows_left)
  chunk <- fread(file_path_predictions, nrows = nrows_to_read, skip = skip, header = (chunk_number == 1))
  chunks[[chunk_number]] <- chunk
  chunk_number <- chunk_number + 1
  skip <- skip + nrows_to_read
}
...
extracted_sites0 <- extracted_sites %>%
  select(-seq_id) %>%
  bind_cols(extr_seq_ids %>% select(seq_id))
...
print(paste("Rows to process:", nrow(extracted_sites0_filtered)))
# =============================================================================
# STEP 3: REFINED MAIN ANALYSIS LOOP (DATA.TABLE - TYPE SAFE)
# =============================================================================
print("--- Step 3: Running Overlap Analysis (Strict 1:1 Mode - Type Safe) ---")

# --- HELPER 1: Manual Reduce Function ---
reduce_dt <- function(df) {
  if (nrow(df) == 0) return(data.table(chr=character(), start=numeric(), end=numeric()))
  
  dt <- as.data.table(df)
  # Force chr to character inside helper to be safe
  dt[, chr := as.character(chr)]
  
  setorder(dt, chr, start, end)
  
  dt[, cum_max_end := cummax(end), by = chr]
  dt[, prev_cum_max := shift(cum_max_end, fill = -1, type = "lag"), by = chr]
  dt[, new_block := start > prev_cum_max]
  dt[, block_id := cumsum(new_block), by = chr]
  
  reduced <- dt[, .(start = min(start), end = max(end)), by = .(chr, block_id)]
  reduced[, block_id := NULL]
  
  return(reduced)
}

# --- HELPER 2: Count Overlaps ---
count_overlaps_dt <- function(truth_dt, pred_dt) {
  if (nrow(truth_dt) == 0 || nrow(pred_dt) == 0) return(0)
  
  # Safety: Ensure both chr columns are character
  truth_dt[, chr := as.character(chr)]
  pred_dt[, chr := as.character(chr)]
  
  truth_dt[, truth_id := .I]
  
  # Non-Equi Join
  overlaps <- truth_dt[pred_dt, 
                       on = .(chr, start <= end, end >= start), 
                       nomatch = NULL]
  
  return(uniqueN(overlaps$truth_id))
}

# --- MAIN LOOP ---

# 1. Prepare True Sites (Force chr to character)
dt_true_sites <- as.data.table(true_binding_sites)
dt_true_sites[, chr := as.character(chr)] 

tf_columns <- setdiff(colnames(extracted_sites0_filtered), "seq_id")

tf_results <- lapply(tf_columns, function(col_name) {
  
  # 2. Prepare Prediction Data
  tf_family_pattern <- gsub("_tnt", "", col_name)
  
  # Note: 'separate' creates character columns by default, but we ensure consistency
  current_tf_data <- extracted_sites0_filtered %>%
    dplyr::select(seq_id, prediction_score = all_of(col_name)) %>%
    distinct(seq_id, .keep_all = TRUE) %>%
    separate(seq_id, into = c("chr", "loc"), sep = ":", remove = FALSE) %>%
    separate(loc, into = c("start", "end"), sep = "-") %>%
    mutate(start = as.numeric(start), end = as.numeric(end)) %>%
    as.data.table()
  
  # Force chr to character
  current_tf_data[, chr := as.character(chr)]
  
  # 3. Filter Common IDs
  matching_common_ids <- common_ids0_filtered %>%
    filter(
      seq_id %in% current_tf_data$seq_id &
        grepl(tf_family_pattern, label, ignore.case = TRUE)
    )
  
  ipm_sites_dt <- current_tf_data[seq_id %in% matching_common_ids$seq_id]
  
  # 4. Identify True Sites for this TF
  matches_indices <- grep(
    tf_family_pattern,
    dt_true_sites$label,
    ignore.case = TRUE
  )
  
  if (length(matches_indices) == 0) return(NULL)
  
  # --- DATA PREPARATION (REDUCTION) ---
  
  # A. Reduce True Sites
  current_true_subset <- dt_true_sites[matches_indices, .(chr, start, end)]
  current_true_reduced <- reduce_dt(current_true_subset)
  
  # B. Reduce Predicted Sites
  if (nrow(ipm_sites_dt) > 0) {
    ipm_sites_reduced <- reduce_dt(ipm_sites_dt[, .(chr, start, end)])
  } else {
    ipm_sites_reduced <- data.table(chr=character(), start=numeric(), end=numeric())
  }
  
  # C. Reduce Bound Predictions
  ipm_bound_subset <- ipm_sites_dt[prediction_score > 0.5]
  
  if (nrow(ipm_bound_subset) > 0) {
    ipm_bound_reduced <- reduce_dt(ipm_bound_subset[, .(chr, start, end)])
  } else {
    ipm_bound_reduced <- data.table(chr=character(), start=numeric(), end=numeric())
  }
  
  # --- CALCULATE METRICS ---
  
  true_ipm_hits <- count_overlaps_dt(current_true_reduced, ipm_sites_reduced)
  n_pred_bound_true <- count_overlaps_dt(current_true_reduced, ipm_bound_reduced)
  
  message(
    "TF: ", tf_family_pattern,
    " | Pred: ", nrow(ipm_sites_reduced),
    " | True: ", nrow(current_true_reduced),
    " | Hits: ", true_ipm_hits
  )
  
  data.frame(
    TF_Family = col_name,
    Total_IPM_Unique_Events = nrow(ipm_sites_reduced),
    Total_IPM_Predicted_Bound = nrow(ipm_bound_reduced),
    True_IPM_Hits = true_ipm_hits,
    True_Hits_Predicted_Bound = n_pred_bound_true,
    True_Hits_Predicted_NotBound = max(0, true_ipm_hits - n_pred_bound_true),
    stringsAsFactors = FALSE
  )
})

Detailed_Stats <- dplyr::bind_rows(tf_results)

# =============================================================================
# STEP 4: EXPORT (Same as before)
# =============================================================================
print("--- Result Preview ---")
print(head(Detailed_Stats))
write.csv(Detailed_Stats, output_file_path, row.names=FALSE)
print(paste("Done! File saved to:", output_file_path))